...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Policy is discrete, markets are continuous: Central banks and governments move in jumps (rate decisions, QE, capital controls). Markets must reprice entire distributions after the fact.
...
\item Cross-asset linkages are under-arbitraged: Many investors specialise in a single asset class. Macro traders operate between silos.
\end{enumerate}
...
\item CTAs (Commodity Trading Advisors)\\
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Directional Trades: Betting on an asset moving in a particular direction. Example: going long copper on a bullish commodities view.
\item Relative Value Trades: Pairing or grouping assets to capture the relative value differential between them. Example: during European crisis, short Italian 5-year BTPs vs.\ long German Bobls, betting Italy will see yields rise relative to Germany.
\item Thematic Trades: Longer-term positions based on structural macro views.\\
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Conventional Policy Regime: Policy rate is the main tool. Yield curve responds normally. Transmission is through rates to credit, to growth, then to inflation.
\item Unconventional Policy Regime: Policy rate loses effectiveness (zero lower bound). Balance sheet becomes the primary tool. Transmission shifts to asset prices, expectations, and FX.
\item Crisis/Emergency Regime: Stability is more important than inflation targeting. Rules are suspended. Liquidity provision dominates all other objectives.
\end{enumerate}
\end{remark}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Price Stability: Most central banks target inflation around $2\%$
\item Full Employment: Explicit in Fed's dual mandate; implicit elsewhere
\item Financial System Stability: Lender of last resort function; systemic risk monitoring
\item Currency Stability: Explicit for some (SNB, PBOC); implicit for others
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Policy Rate: Set by changing overnight rate at which banks lend and borrow to meet reserve requirements. The primary tool in normal times.
\item Reserve Ratio: Raising reserve ratio decreases money multiplier and contracts money supply.
\item Open Market Operations (OMO): Buying and selling government bonds in local currency. Repos are short-term agreements where the central bank agrees to repurchase securities within a specified time.
\item Currency Intervention:
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Monetary Base: Sum of currency in circulation and reserves held at the central bank. Can only be controlled via open market operations.
\item Money Supply: Changes in money supply are leading predictors of GDP growth and inflation. The Quantity Theory of Money:
\begin{equation*}
M \times V = P \times Y
\end{equation*}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Hawks: Wary of inflation, favour tighter policy, defend currency value
\item Doves: Favour growth and employment, tolerate higher inflation, support looser policy
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Statements: Released after each meeting; market moves on hawkish/dovish tone changes
\item Minutes: Released with lag ($2$ to $3$ weeks); reveal vote splits and internal debates
\item Press Conferences: Chair/President Q\&A; tone and body language matter
\item Projections: Dot plots (Fed), staff forecasts; reveal expected rate path
\item Speeches: Individual members signalling; watch for coordinated messaging
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Federal Reserve (Fed): Dual mandate (price stability, full employment). FOMC meets $8$ times/year with $12$ voting members ($7$ governors, NY Fed, $4$ rotating).\\
Most important central bank globally due to USD reserve status.
\item European Central Bank (ECB): Price stability mandate (HICP $<2\%$). Governing Council of $25$ members. Key rates: Main Refinancing Operations (MRO), deposit facility rate. Tools include LTRO, TLTRO, OMT, APP.
\item Bank of Japan (BOJ): Targets $2\%$ inflation (chronically undershoots). Policy Board of $9$ members. Pioneer of QE, yield curve control (YCC), negative rates.
\item Bank of England (BOE): $2\%$ inflation target. MPC of $9$ members with individual accountability. First major central bank to publish inflation forecasts.
\item Swiss National Bank (SNB): Inflation target $<2\%$. Notable for 2011--2015 EUR/CHF floor at $1.20$ and subsequent abandonment. Massive FX reserve accumulation.
\item People's Bank of China (PBOC): Multiple objectives including growth, stability, FX management. Manages CNY within trading band. Uses reserve requirements actively.
\end{enumerate}
\end{remark}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item LTRO/TLTRO: Long-Term Refinancing Operations provide multi-year funding to banks against collateral. $3$-year LTROs in $2011$ to $2012$ injected over EUR$1$ trillion.
\item OMT (Outright Monetary Transactions): Announced September $2012$. Unlimited bond purchases for countries in EFSF/ESM programs. Never actually used but stabilised spreads.
\item APP (Asset Purchase Programme): QE-equivalent. Purchases of government bonds, covered bonds, corporate bonds, ABS.
\item TARGET2: Payment system showing intra-Eurozone imbalances. German TARGET2 claims rose to EUR$700$B+ during crisis, reflecting capital flight from periphery.
\end{enumerate}
\end{remark}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Central Bank Swap Lines: USD liquidity to foreign central banks to prevent LIBOR spikes
\item TAF (Term Auction Facility): $28$--$84$ day unsecured lending without discount window stigma
\item TSLF (Term Securities Lending Facility): Treasury lending against lower-quality collateral
\item CPFF (Commercial Paper Funding Facility): Commercial paper purchases to prevent market freeze
\item TALF (Term ABS Loan Facility): Lending against consumer ABS to support credit markets
\item Agency MBS Purchases: $\$1.25$ trillion in QE1 alone
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Surprise Direction: Above/below consensus drives immediate price action. Magnitude of surprise matters more than absolute level.
\item Revision Risk: Prior releases are frequently revised. Initial release moves markets; revisions move markets again if material.
\item Hierarchy: Not all data is equal. NFP and CPI dominate in the US; PMIs dominate globally due to frequency and forward-looking nature.
\item Regime Dependence: What data matters depends on the regime. In low-inflation regimes, growth dominates. In high-inflation regimes, inflation dominates everything.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item GDP (Headline, QoQ, YoY): Measures total production in current or constant prices (nominal, real GDP). Released quarterly with preliminary, revised, and final prints. Backward-looking but sets narrative.
\item Purchasing Managers' Indices (PMIs): Survey business managers on employment, new orders, inventories, production, supplier deliveries, prices, backlogs, imports, and exports on scale of better/same/worse vs prior month. Above 50 = expansion; below 50 = contraction. Most important leading indicator.
\item Industrial Production: Measures growth in manufacturing, utilities, and mining. More cyclical than services. Released monthly. Leading indicator for GDP forecasting.
\item Trade Volumes, Baltic Dry Index (BDI): Shipping costs for dry bulk cargo. Leading indicator on global trade activity since it cannot be speculated (no futures market).
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Consumer Price Index (CPI)/Core CPI: Prices paid by consumers for basket of goods and services. Core excludes food and energy (volatile). Central banks target core.
\item Producer Price Index (PPI): Average change in selling prices received by domestic producers. Leads CPI by $1$--$3$ months as input costs pass through.
\item PCE (Personal Consumption Expenditures): Fed's preferred inflation measure. Differs from CPI in weights and methodology.
\item Inflation Expectations: Market-based (breakevens from TIPS) or survey-based (Michigan, NY Fed). Central banks watch expectations closely. De-anchoring is a regime change.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Non-Farm Payrolls (NFP): Number of workers in US excluding farms, private households, nonprofits, self-employed, and military. Released first Friday of each month. Among highest-volatility releases for rates, FX, equities.
\item Unemployment Rate: Unemployed as percentage of labor force. Headline number; can be distorted by participation changes.
\item Participation Rate: Labor force as percentage of civilian population. Structural decline (aging) vs cyclical decline (discouraged workers) matters.
\item Wage Growth (Average Hourly Earnings): More important than job count for inflation outlook. 
\item JOLTS (Job Openings and Labor Turnover Survey): Shows job openings, hires, quits. Quits rate indicates labor market tightness and worker confidence.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Current Account: Net exports + net foreign income + transfer payments. Deficit countries need capital inflows or currency depreciation; surplus accumulate reserves and have structurally stronger currencies.
\item Capital Account/Financial Account: Portfolio flows, FDI, banking flows. Can finance current account deficits but creates vulnerabilities (sudden stop risk).
\item Trade Balance: Exports minus imports. Incremental changes in exports or imports can have big impact on trade balance as percentage of GDP.
\item Terms of Trade (ToT): Export prices divided by import prices. Improves when export prices rise relative to imports. Currency depreciation typically worsens ToT.
\item FX Reserves: Central bank holdings of foreign currency and gold. Countries with large reserves can intervene; watching reserve changes indicates intervention.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Budget Balance: Revenues minus expenditures. Deficit = borrowing required. Deficit/GDP ratio matters more than absolute level.
\item Debt-to-GDP: Stock measure of accumulated borrowing. Matters more for EM and Euro periphery than for reserve currency issuers.
\item Primary Balance: Budget balance excluding interest payments. Shows underlying fiscal stance; positive primary balance means debt/GDP can stabilise if growth exceeds interest rate.
\item Debt Sustainability: Interest payments as \% of revenue. Rising share means deteriorating sustainability.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Retail Sales: Consumer demand for finished goods. Durable vs non-durable breakdown matters.
\item Consumer Confidence: Survey of expected financial situation. Conference Board and Michigan surveys in US. Leading indicator, confidence leads spending, but noisy.
\item Personal Income and Spending: Income growth enables spending growth. Savings rate = income minus spending.
\item Savings Rate: Higher savings = lower current consumption but more sustainable. US savings rate has declined structurally; Asian savings rates remain high.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Building Permits: Lead housing starts since permits are required before construction. Most forward-looking housing indicator.
\item Housing Starts: Counted when construction begins. Gauge of housing demand and construction activity.
\item Existing/New Home Sales: Volume of transactions. Prices vs volumes can diverge.
\item Home Price Indices: Case-Shiller, FHFA. Lag actual market conditions.
\item Household Formations: Structural driver of housing demand. Post-crisis decline reduced demand.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Manufacturing vs Services PMI: Manufacturing is more cyclical and globally linked. Services dominate GDP but move slower.
\item New Orders: Most forward-looking PMI subcomponent. Rising new orders = future production.
\item Inventories: High inventories relative to sales = future production cuts. Inventory cycles matter for commodities and industrial equities.
\item Capacity Utilisation: Actual output relative to potential. Over $100\%$ = overheating, inflation risk. Well under $100\%$ = slack, deflation risk.
\item Durable Goods Orders: Capital expenditure intentions. Core capital goods orders (ex-aircraft, ex-defence) is key business investment indicator.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Working-Age Population ($15$--$64$): Drives potential growth. Many developed countries have peaked or will peak soon.
\item Prime Saving Age ($35$--$69$): Peak saving years. Countries past this peak see declining savings and current accounts.
\item Dependency Ratio: Non-working population (young + old) divided by working population. Higher ratio = more fiscal pressure, lower savings.
\item Median Age: Summary statistic for demographic structure. Europe/Japan: 40s; US: 38; EM: $20$s--$30$s.
\item Retirement Age vs Life Expectancy: Gap determines pension system sustainability. Life expectancy rising; retirement ages barely moving.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Already peaked: Germany ($2006$), Japan ($2016$), Netherlands ($1989$), UK ($2009$), US ($2007$)
\item Peaking soon: China ($2032$), Korea ($2023$), Russia ($2025$), Spain ($2022$)
\item Long runway: India ($2050$), Indonesia ($2050$), Nigeria ($2050$), Philippines ($2050$)
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item First Friday: Employment Report (NFP, unemployment rate, wages)
\item Mid-month: CPI, Retail Sales, Industrial Production
\item Third week: Housing Starts, Building Permits
\item Late month: GDP (quarterly), PCE, Durable Goods
\item First business day: ISM Manufacturing PMI
\item Third business day: ISM Services PMI
\end{enumerate}
