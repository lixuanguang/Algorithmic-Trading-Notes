...
\item Fixed Income (FI) (Policy and Growth Sensor): encodes growth expectations, inflation expectations, central bank credibility. Yield curves summarise the entire macro narrative; steepening means growth/reflation; flattening or inversion means slowdown/recession. Fixed income tells what the market thinks will happen before it happens.
\item Foreign Exchange (FX) (Adjustment Mechanism): FX reflects relative, not absolute, macro conditions. Sensitive to rate differentials, capital flows, balance of payments, risk-on/risk-off regimes. FX absorbs and redistributes macro shocks across countries.
\item Equities (Growth Expression): equities are long growth, long liquidity, short uncertainty. Equity indices aggregate earnings expectations, discount rates, risk appetite. Volatility (VIX) is a second-order macro signal, not just an equity signal. Equities express the confidence (or fear) embedded in the macro outlook.
\item Commodities (Inflation and Real-Economy Link): driven by physical supply and demand, inventories, geopolitics. Highly sensitive to growth cycles, inflation shocks, currency moves (especially USD). Commodities anchor macro views in real economic constraints.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Growth Shocks: rates rise, equities rally, cyclicals outperform, commodities rise. FX favours growth-linked currencies (AUD, CAD, EM FX).
\item Recession Shocks: rates fall, curves flatten/invert, equities sell off, USD strengthens (flight to safety), commodities weaken (except gold).
...
\item Liquidity Shocks: USD strengthens sharply, credit spreads widen, correlations spike toward 1, volatility explodes. Cross-currency basis blows out.
\end{enumerate}
...
Position sizing aims to adjust each position for volatility. This normalises all positions such that losses are more predictable and have the same probability of occurring:
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
...
\begin{remark} \hlt{Risk Utilisation}\\
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Sharpe Ratio: $\frac{E[R] - R_f}{\sigma}$, risk-adjusted return; higher is better. Assumes normal distribution.
\item Sortino Ratio: Sharpe with only downside volatility, differentiating between good and bad volatility.
\item Maximum Drawdown: peak-to-trough decline; critical for understanding worst-case capital loss.
\item Value at Risk (VaR): $95\%$ or $99\%$ daily VaR (expected loss exceeded on $1$ in $20$ or $100$ days).
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Growth view: rates (duration), equities (beta), FX (pro-cyclical currencies), commodities (cyclicals)
\item Inflation view: commodities (energy, agriculture), breakevens, FX (commodity currencies), TIPS
\item Crisis/Deflation view: bonds (duration), USD/JPY/CHF (havens), volatility (VIX, swaptions), gold
\item Policy divergence view: FX (rate differentials), yield curve spreads, relative equity indices
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Free-floating: price discovery happens in FX
\item Managed/pegged: pressure builds elsewhere such as in rates, reserves, capital controls
\item Hard pegs/Currency boards: FX looks stable until it breaks violently
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Money Markets: Short-term instruments (maturity $<1$ year). Includes T-bills, commercial paper, repos, Fed Funds, SOFR.
\item Government Bonds: Sovereign debt across the curve ($2$Y, $5$Y, $10$Y, $30$Y). Risk-free rate benchmark.
\item Inflation-Linked Bonds: TIPS in US, Linkers in UK, OATi in France. Index to CPI.
\item Corporate Bonds: Investment grade (IG) and high yield (HY). Credit spread over risk-free rate.
\item Municipal Bonds: State and local government debt, often tax-exempt.
\item Mortgage-Backed Securities (MBS): Agency and non-agency. Prepayment risk.
\item Asset-Backed Securities (ABS): Auto loans, credit cards, student loans.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Treasury Bills (T-Bills): Short-term US government obligations ($4$-week, $13$-week, $26$-week, $52$-week). Risk-free benchmark.
\item Commercial Paper: Unsecured short-term corporate debt, typically $90$--$270$ days. Lower rates than bank borrowing.
\item Repurchase Agreements (Repos): Sale of securities with agreement to repurchase. Critical for short-term funding and leverage.
\item Certificates of Deposit (CDs): Bank time deposits, $30$ days to $5$ years.
\item Banker's Acceptances: Promissory notes issued by firms to banks, typically for trade finance.
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Fed Funds Rate: Unsecured overnight lending rate between depository institutions.
\item SOFR (Secured Overnight Financing Rate): Overnight Treasury repo rate as USD benchmark.
\item LIBOR (Legacy): London Interbank Offered Rate. Historically the most important rate in global finance. Phased out post-$2021$ due to manipulation scandals.
\item EONIA/EURSTR: Euro overnight rates. EURSTR replaced EONIA in $2022$.
\item SONIA: Sterling overnight rate for GBP.
\end{enumerate}
...
\end{enumerate}
Example: If BOE rate is $0.50\%$ and December MPC OIS trades at $0.625\%$, the market prices $50\%$ chance of a $25$bp hike.
\end{remark}
...
\item Colour codes by year: White (Y$1$), Red (Y$2$), Green (Y$3$), Blue (Y$4$), Gold (Y$5$)
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Fixed-for-Floating: Most common. Exchange fixed rate for floating rate (SOFR, EURSTR).
\item Receiver: Receives fixed, pays floating. Profits if floating rates fall.
\item Payer: Pays fixed, receives floating. Profits if floating rates rise.
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Schatz (DU): $1.75$--$2.25$ year maturity
\item Bobl (OE): $4.5$--$5.5$ year maturity
\item Bund (RX): $8.5$--$10.5$ year maturity
\item Buxl (UB): $24$--$35$ year maturity
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Steepener: Long short-end, short long-end. Profits from curve steepening.
\item Flattener: Short short-end, long long-end. Profits from curve flattening.
\item Bull Flattener: Rates fall, long end falls more than short end.
\item Bear Flattener: Rates rise, short end rises more than long end (often Fed hiking).
\item Bull Steepener: Rates fall, short end falls more than long end (often Fed cutting).
\item Bear Steepener: Rates rise, long end rises more than short end (inflation fears).
\end{enumerate}
The $2$s$10$s spread ($10$Y yield minus $2$Y yield) is a classic recession indicator. Inversion has preceded every US recession since $1970$.
\end{remark}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Carry: Coupon income $-$ financing cost. $+$ for steepeners, $-$ for flatteners in normal yield curve.
\item Rolldown: Return from bond "rolling down" a positively sloped yield curve as it approaches maturity.
...
\item $5$Y, $10$Y, and $30$Y maturities issued quarterly
...
\end{equation*}
For example, if $10$Y Treasury yields $4.0\%$ and $10$Y TIPS yields $1.5\%$, $10$Y breakeven is $2.5\%$.
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Reference Entity: Legal name for CDS purposes (e.g., Hellenic Republic for Greece)
\item Curve Inversion: When short-term yields exceed long-term yields, market prices default risk
\item Credit Events: Failure to pay, restructuring, moratorium
\item Recovery Rate: Typically assumed $40\%$ for sovereigns; historical average $55\%$ with high variance
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Protection Buyer: Pays spread (premium), receives par if credit event occurs
\item Protection Seller: Receives spread, pays par minus recovery if credit event occurs
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Failure to Pay: Sovereign/corporate misses interest or principal payment
\item Restructuring: Postponement, reduction, or deferral of obligations
\item Bankruptcy: Corporate only (sovereigns cannot file bankruptcy)
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Russia $1998$: Devaluation and default on local currency obligations, $\$73$B
\item Argentina $2001$: Largest sovereign default at time, $\$82$B, $70\%$ haircut
\item Uruguay $2003$: Contagion from Argentina, 5-year maturity extension
\item Greece $2012$: PSI restructuring, largest sovereign default in history, $\$200$B+
\item Argentina $2020$: Ninth default, $\$65$B restructured
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Treasury: SHY ($1$--$3$Y), IEF ($7$--$10$Y), TLT ($20+$Y), TBT ($2$x inverse $20+$Y)
\item TIPS: TIP (broad TIPS)
\item Investment Grade: LQD (corporate IG), AGG (aggregate bond)
\item High Yield: HYG, JNK (high yield corporate)
\item Emerging Markets: EMB, PCY (EM sovereign debt)
\item Municipals: MUB (muni bonds)
\item MBS: MBB (mortgage-backed)
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Modified Duration: Approximate percentage price change for $1\%$ yield change
\item DV01 (Dollar Value of 01): Dollar change for 1bp yield change = $\frac{\text{Duration} \times \text{Price}}{10000}$
...
\end{enumerate}
A $10$Y bond with duration $8$ loses approximately $8\%$ if yields rise $100$bp.
\end{remark}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item 2s10s inversion: $10$Y yield below $2$Y yield
\item 3m10s inversion: $10$Y yield below $3$-month T-bill
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Growth Shock (positive): Rates rise, curve steepens, credit spreads tighten
\item Recession: Rates fall sharply, curve bull steepens then flattens, credit spreads widen
\item Inflation Shock: Nominal rates rise, real rates may fall initially, curve bear steepens, TIPS outperform nominals
\item Deflation/Disinflation: Nominal rates fall, breakevens collapse, duration rallies
\item Financial Crisis: Flight to quality (Treasuries rally), credit spreads explode, funding markets seize
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Contango: Near-term futures trade cheaper than further-dated contracts. Exists due to storage costs, financing costs, and opportunity cost of capital. Normal state for storable commodities like gold.
\item Backwardation: Near-term futures trade at premium to further-dated contracts. Occurs during supply shortages or immediate demand spikes.
\item Agricultural seasonality: Curves can exhibit both. Contango after harvest (abundant supply, storage costs), backwardation before harvest (depleted stocks, tight supply).
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item CRB (Commodity Research Bureau) Index: $22$ commodities, broad representation of prices.
\item S\&P GSCI: Goldman Sachs Commodity Index. Production-weighted, heavily energy-exposed ($>50\%$).
\item Bloomberg Commodity Index (BCOM): Diversified, liquidity-weighted.
\item ETFs: DBC (Deutsche Bank commodities, $>50\%$ energy), DBA (agriculture only, includes corn, soybeans, sugar, cattle, coffee, cocoa).
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Quality measures: API gravity (higher = lighter) and sulphur content (lower = sweeter)
\item Light sweet crude: Most desirable (WTI, Brent). Heavy sour crude least desirable (Venezuela, Mexico)
\item Products: Gasoline (cars), kerosene (diesel, jet fuel), heating oil, LPGs
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item WTI (West Texas Intermediate): Light sweet crude. Trades on NYMEX (CL). Delivery at Cushing, Oklahoma. Contract: 1,000 barrels. ETF: USO.
\item Brent Crude: North Sea light sweet crude. Trades on ICE (CO). Prices more than half of world's internationally traded crude. Delivery at Sullom Voe, Scotland.
\item WTI-Brent Spread: Historically WTI traded at premium (higher grade), but infrastructure constraints at Cushing and Middle East supply sensitivity pushed Brent to premium.
\end{enumerate}
\end{remark}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Strait of Hormuz: Most important chokepoint. $17$ million bbl/d ($20\%$ of global traded oil). Between Iran and Oman. Geopolitical flashpoint.
\item Strait of Malacca: $14$ million bbl/d. Connects Indian and Pacific Oceans. Critical for Asian supply.
\item Mandab Strait/Suez Canal: 3 million and 2 million bbl/d respectively. Piracy risk.
\item Pipelines: Druzhba (Russia to Europe, 2,300 miles, 1.4 million bbl/d). Keystone (Canada to US).
\end{enumerate}
...
\item $3:2:1$ Crack Spread: $3$ barrels crude vs.\ $2$ barrels gasoline + $1$ barrel heating oil (most common)
...
\item Shale revolution: Horizontal drilling and hydraulic fracturing transformed US from importer to exporter
\item Flaring: Burning unwanted natural gas, common in US due to regulatory requirements
\item LNG: Liquefied at $-260^\circ$F for transport, 600x less volume than gas state
\item Uses: Power generation, heating, fertiliser (ammonia), transportation
\end{enumerate}
...
\item Real rates: Gold inversely correlated with real interest rates. Negative real rates support gold.
\end{enumerate}
...
\end{enumerate}
Trading: SLV ETF ($1$ oz per share), silver futures (SI) on COMEX ($5,000$ troy oz). "Triple $9$s" ($99.9\%$ purity) is deliverable grade.
\end{remark}
...
\item Ethanol mandate: US Energy Policy Act $2005$ requires ethanol blending, creating structural demand
...
\item Classifications: season (winter/spring), gluten content (hard/soft), colour (red/white/amber)
...
\item Brazil/Argentina weather critical, neighbouring countries, correlated weather risk
\end{enumerate}
...
\item Classifications: character (strength), grade (colour/purity), staple (fibre length)
\item More drought-resistant than other crops
\end{enumerate}
Trading: ICE (CT), cents/lb, $50,00$0 lbs/contract ($\approx 104$ bales). $1$ bale = $480$ lbs.\\
...
\item Biennial cycle: Coffee trees alternate high and low yield years
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Growth Shock: Commodity demand rises, prices increase, especially energy and industrial metals
\item Recession: Demand destruction, prices fall (except gold as safe haven)
\item Inflation Shock: Commodities outperform as inflation hedge; energy and agriculture lead
\item USD Weakness: Commodity prices rise (inverse correlation with dollar)
\item Geopolitical Crisis: Energy spikes on supply fears; gold rises on safe-haven demand
\end{enumerate}
