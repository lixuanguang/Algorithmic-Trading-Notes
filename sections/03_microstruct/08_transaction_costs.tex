...
\begin{remark} \hlt{Trading Decision Framework}\\
Best execution depends on benchmark, investor's level of risk aversion, and their overall goals.\\
Decision Process from Buy-Side Trader Perspective:
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Portfolio manager initially notifies trader of the order
\item If there are any specific restrictions, trader must use the designated broker
\item Otherwise, trader must assess how difficult the order will be to trade
\item For orders that will provide much needed liquidity to the markets, trader should strive for optimal price
\item For orders judged easy, trader has leeway in how best to deal with them
\item Tough orders may be sub-categorised based on whether:
\begin{enumerate}[label=\arabic*.]
\setlength{\itemsep}{0pt}
\item They are a large percentage of average daily volume (ADV)
\item The asset is exhibiting significant trading momentum
\item The investor has flagged the order as urgent
\end{enumerate}
\item Depending on perceived difficulty, trader must select most appropriate method of trading: using trading algorithms, DMA, trying to cross the order, or negotiating a principal transaction with a dealer
\end{enumerate}
\end{remark}

\begin{remark} \hlt{Order Difficulty Assessment}\\
Pre-trade analysis enables quantification of order difficulty using historical TCA results.
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Order Size: anything over $20$-$25\%$ of ADV is considered large and difficult; less than $1\%$ is small
\item Liquidity: proxied by market capitalisation ($>\$10$B, $\$1$-$10$B, $\$100$M-$1$B, $<\$100$M) and turnover
\item Volatility: highly illiquid, high volatility equities cost $400$-$1200$bps vs. $10$-$40$ bps for liquid, low volatility
\item Price Momentum: persistent trends (favourable or adverse) affect cost and strategy selection
\item Urgency: aggressive trades over small time horizon will cause significant market impact
\item Trading Horizon: shorter trading horizon makes order execution more difficult
\end{enumerate}
\end{remark}
