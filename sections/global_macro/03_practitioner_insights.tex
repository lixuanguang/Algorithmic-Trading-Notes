...
\item First trade: Long Finnish markka---disaster, FIM devalued again in early September 1992
...
\item Compare fundamental view to market perception---that's where you find trades
...
\item For currencies: Current account deficit currency going up = longs building; surplus currency going down = shorts building
...
\item ``If your views are a little confused, you really want to back off on your trades''
\end{enumerate}

\textbf{The ``Favourite Trade'' Concept}
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Interview question: ``If you had to do one trade to get rich today, what would it be?''
\item Reveals: Does the person think big? A 2\% potential = never a big player; 20--30\% potential = getting somewhere
\item Captures basic contours of thought process
\item Looking for people who can see the big move
\end{enumerate}
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Good analyst finds the trade or identifies pitfalls
\item Good trader manages the risk and runs the position
\item Key difference is in ``their stomachs''---ability to manage large position sizes is rare
\item ``My skill is in finding trades, not managing them''
\item Having trades on makes you profoundly honest---forces you to think ahead
\end{enumerate}
...
\item ``If you're going to panic in markets, panic early. Panicking late is a recipe for disaster''
\item After big losses, take a month away---``brain damage'' from recent events impairs judgment
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item ``The idea that speculators broke the Bank of England in 1992 is ludicrous''
...
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Strong views but differentiate views from trades
\item Ability to change their mind
...
\item ``Domestics misunderstand their own economies''
\item Ask an equity guy about bonds to get fresh perspective
\end{enumerate}

\textbf{Evolution of Global Macro}
\begin{enumerate}[label=\roman*.]
\setlength{\itemsep}{0pt}
\item Instruments changed (intra-European currencies eliminated)
\item More specialisation: currency, relative value, bond, option specialists
\item More capital = more people = bifurcation between superstars and commoditised managers
\item Superstars can charge 3--4\% management fees; rest becoming commoditised
\end{enumerate}

Best global macro trader worked with: Jim Leitner---combines intellectual ability, curiosity, open mind, and ``the stomach to carry all of that in size.''
